...
Note that if $A$ is $m\times n$, then the null space of $A$ is a subset of $\RR^n$. It is important for us to check that the null space is really a subspace of $\RR^n$, that is, closed under scaling and addition: if $A\mathbf{x}=0$ then $A\alpha\mathbf{x}=\alpha A\mathbf{x}=\mathbf{0}$ too; if $A\mathbf{x}=\mathbf{0}$ and $A\mathbf{w}=\mathbf{0}$, then we have $A(\mathbf{x}+\mathbf{w})=A\mathbf{x}+A\mathbf{w}=\mathbf{0}$ too.
...
Recall that the solutions to $A\mathbf{x}=\mathbf{0}$ all lie in the \emph{null space}, which is a vector subspace of $\RR^n$. It turns out that solutions to the system $A\mathbf{x}=\mathbf{0}$ all lie in a \emph{translate} of the null space.
...
\[2x+3y=0
\]
The solutions to this equation form a line through the origin, a subspace of $\RR^2$. We can plot it: the line through the origin with slope $-2/3$.
