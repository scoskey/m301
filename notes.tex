...
Given a system of linear equations $A\mathbf{x}=\mathbf{b}$, if it is consistent we know how to solve it. If it is inconsistent, we cannot find any solution. But can we still find a \emph{best approximate solution} $\mathbf{x}$ that makes $A\mathbf{x}$ as close as possible to $\mathbf{b}$? In this section we show how to find best approximate solutions using orthogonality.

Geometrically, if $A\mathbf{x}=\mathbf{b}$ is inconsistent, this means that $\mathbf{b}$ doesn't lie in the column space of $A$. We want to replace $\mathbf{b}$ with the closest possible vector $\mathbf{p}$ that \emph{is} in the column space of $A$, and then solve $A\mathbf{x}=\mathbf{p}$ instead. From a picture [draw], we see that an optimal solution is achieved exactly when the error vector $\mathbf{b}-A\hat{\mathbf{x}}$ is orthogonal to the column space of $A$.

The method is known as \emph{orthogonal projection}.
\begin{itemize}
...
\begin{example}
  Find the best approximate solution to the inconsistent system
...
The situation is somewhat simpler when $A$ is just a single vector $\mathbf{a}$. Here, we are projecting $\mathbf{b}$ onto the line through $\mathbf{a}$.
\begin{itemize}
...
\begin{example}
  $\mathbf{a}=(1,2,3)$ and $\mathbf{b}=(4,6,4)$.
\end{example}

Given a matrix $A$, we can always find the \emph{projection matrix} $P$ which maps vectors $\mathbf{b}$ to their projections $\mathbf{p}$ in the column space of $A$. Recalling that $A^TA\hat{\mathbf{x}}=A^T\mathbf{b}$ and $\mathbf{p}=A\hat{\mathbf{x}}$, we find that
  \[P=A(A^TA)^{-1}A^T
  \]

\begin{example}
  Returning to the previous example, we find $P$...
\end{example}
