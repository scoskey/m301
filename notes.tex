...
In the last section we introduced vector spaces, and introduced the column space of a matrix. There are several more subspaces that can be associated with a given matrix $A$, and in this session we investigate the second most important one, the null space.

\begin{defn}
  Given a matrix $A$, the \emph{null space} of $A$ is the set of all $\mathbf{x}$ such that $A\mathbf{x}=\mathbf{0}$.
\end{defn}

In function terminology, the column space of $A$ is the range, and the null space is the set of zeros or roots.

Note that if $A$ is $m\times n$, then the null space of $A$ is a subset of $\RR^n$. It is also important for us to check that the null space is really a subspace of $\RR^n$. For example if we have $A\mathbf{x^1}=\mathbf{0}$ and $A\mathbf{x^2}=\mathbf{0}$, then we have $A(\mathbf{x^1}+\mathbf{x^2})=\mathbf{0}$ too.

In the following example we show how to calculate the null space of a matrix.
...
Since the parameters are in some sense unnecessary information, we often write just the \emph{null space matrix} which is just the matrix of special solutions.
\[N=\begin{bmatrix}
    -2&0\\0&-2\\1&0\\0&1
  \end{bmatrix}
\]
...
Each special solution provides a recipe for a combination of the columns of $A$ that gives $\mathbf{0}$.
