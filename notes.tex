...
Matrices do not need to be square. We say a matrix is $a\times b$ if it has $a$ rows and $b$ columns. (We \emph{always} write rows and then columns, not the other way around!) When multiplying matrices, the dot products have to match in length. This means the number of columns of the first matrix must match the number of rows of the second matrix.
\begin{quotation}
