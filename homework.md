...
2. For each of the following systems, carefully graph the equations and the locatin of the solution. Then graph the "vector picture" (the right-hand side as the tip of a parallelogram made from two column vectors). Finally, find the solution.
  * $\set{x-2y=0;\ x+y=6}$
...
3. Consider a system with two equations and three variables $x,y,z$, fill in the blanks. Explain your reasoning with one complete sentence for each answer.
  * The graph of the system will show (2 / 3) (lines / planes?) in (2 / 3)-dimensional space.
